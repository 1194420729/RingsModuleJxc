...
namespace JxcSale
{
    public class SaleOrder : MarshalByRefObject
    {
        private string billname = "saleorder";
        ...
        public Object SaleOrderQuery(string parameters)
        {
            ...
            string sqlCount = cte+"select count(0) as cnt from cte "
                        + sb.ToString();
        ...
        public Object Delete(string parameters)
        {
            ...
            using (DBHelper db = new DBHelper())
            {
                ...
                {
                    var salebill = db.FirstOrDefault("select * from bill where content->'details' @> '[{\"saleorderid\":" + s + "}]'");
                    if (salebill != null)
                    {
                ...
                if (options.content.Value<bool>("strictordermanage"))
                {
                    foreach (string s in ss)
                    {
                        var saleorder = db.First("bill", Convert.ToInt32(s));
                        if (saleorder.content["auditorid"] != null)
                        {
                            saleorder.UnModifyVirtualStorage(db);
                        }
        ...
        public Object Abort(string parameters)
        {
            ...
            using (DBHelper db = new DBHelper())
            {
                ...
                foreach (string s in ss)
                {
                    var saleorder = db.First("bill", int.Parse(s));
                    if (saleorder.content["auditorid"] == null)
                    {
                        return new { message = StringHelper.GetString("草稿无需中止，可以直接删除") };
                    }

                    if (saleorder.content.Value<bool>("abort") == true)
                    {
                ...
                {
                    var saleorder = db.First("bill", Convert.ToInt32(s));
                    saleorder.content["abort"] = true;
                    db.Edit("bill", saleorder);

                    if (options.content.Value<bool>("strictordermanage"))
                    {
                        saleorder.ModifyVirtualStorageForAbort(db);
                    }
                }
        ...
        public Object UnAbort(string parameters)
        {
            ...
            using (DBHelper db = new DBHelper())
            {
                ...
                foreach (string s in ss)
                {
                    ...
                    if (saleorder.content.Value<bool>("abort") == false)
                    {
                ...
                foreach (string s in ss)
                {
                    ...
                    saleorder.content.Remove("abort");
                    db.Edit("bill", saleorder);
                    if (options.content.Value<bool>("strictordermanage"))
                    {
                        saleorder.ModifyVirtualStorageForUnAbort(db);
                    }
                }
        ...
        public Object Audit(string parameters)
        {
            ...
            using (DBHelper db = new DBHelper())
            {
                ...
                foreach (string s in ss)
                {
                    ...
                    saleorder.content["auditorid"] = PluginContext.Current.Account.Id;
                    saleorder.content["audittime"] = DateTime.Now;

                    db.Edit("bill", saleorder);

                    var billaudit = saleorder.GetBillAudit(db);
        ...
        public Object BillComment(string parameters)
        {
            ...
            using (DBHelper db = new DBHelper())
            {
                var saleorder = db.First("bill", id);
                saleorder.content["comment"] = comment;
                db.Edit("bill", saleorder);
                db.SaveChanges();
            }
            return new { message = "ok" };
        ...
        public Object Copy(string parameters)
        {
            ...
            using (DBHelper db = new DBHelper())
            {
                foreach (string s in ss)
                {
                    var saleorder = db.First("bill", int.Parse(s));
                    saleorder.id = 0;
                    saleorder.content["billcode"] = BillHelper.GetBillCode("saleorder", db);
                    saleorder.content["billdate"] = DateTime.Now.ToString("yyyy-MM-dd");
                    saleorder.content["makerid"] = PluginContext.Current.Account.Id;
                    saleorder.content["createtime"] = DateTime.Now;
                    saleorder.content["auditstatus"] = 0;
                    saleorder.content.Remove("auditorid");
                    saleorder.content.Remove("audittime");
                    saleorder.content.Remove("abort");
                    foreach (var item in saleorder.content.Value<JArray>("details").Values<JObject>())
                    {
                        item.Remove("deliveryqty");
                        item["uuid"] = Guid.NewGuid().ToString("N");
                    }
                    db.Add("bill", saleorder);
                }
        ...
        public Object RefreshMVWSaleOrder(string parameters)
        {
        ...
        public Object SaleOrderProductReport(string parameters)
        {
            QueryParameter para = ParameterHelper.GetQueryParameters(parameters);
        ...
        public Object InitCustomerReport(string parameters)
        {
            bool showcost = PluginContext.Current.Account.IsAllowed("showcost");
            var option = new DBHelper().First("select * from option");
            int digit = option.content["digit"] == null ? 2 : option.content.Value<int>("digit");
        ...
        public Object SaleOrderCustomerReport(string parameters)
        {
            QueryParameter para = ParameterHelper.GetQueryParameters(parameters);
        ...
        public Object SaleOrderEmployeeReport(string parameters)
        {
            QueryParameter para = ParameterHelper.GetQueryParameters(parameters);
        ...
        public Object SaleOrderStockReport(string parameters)
        {
            QueryParameter para = ParameterHelper.GetQueryParameters(parameters);
        ...
        public Object SaleOrderDetailReport(string parameters)
        {
            QueryParameter para = ParameterHelper.GetQueryParameters(parameters);
