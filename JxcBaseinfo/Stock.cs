...
namespace JxcBaseinfo
{
    public class Stock : MarshalByRefObject
    {
            ...
            try
            {
                ...
                using (DBHelper db = new DBHelper())
                {
                    ...
                    for (int i = 1; i < rowcount; i++)
                    {
                        ...
                        string comment = row.GetCell(2).GetCellValue().ToString();
                        Dictionary<string, string> stock = new Dictionary<string, string>();
                        stock.Add("code", code);
                        stock.Add("name", name);
                        stock.Add("comment", comment);
                        stock.Add("pycode", PyConverter.IndexCode(name));

                        TableModel model = new TableModel()
                        {
                            content = JsonConvert.DeserializeObject<JObject>(JsonConvert.SerializeObject(stock))
                        };
        ...
        public Object Delete(string parameters)
        {
            ...
            using (DBHelper db = new DBHelper())
            {
                var product = db.FirstOrDefault("select * from product where (content->'storage'->'"+id+"'->>'qty')::decimal>0");
                if (product != null)
                {
                    return new { message = "该仓库有库存，不能删除！" };
                }

                var detail = db.FirstOrDefault("select * from bill where (content->>'stockid')::int=" + id 
                    + " or (content->>'stockid2')::int=" + id);
                if (detail != null)
                {
                    return new { message = "该仓库有关联的业务单据，不能删除！" };
                }
