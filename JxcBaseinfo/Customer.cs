    ...
    public class Customer : MarshalByRefObject
    {
        ...
        public Object Delete(string parameters)
        {
            ...
            using (DBHelper db = new DBHelper())
            {
                ...
                if (model.content.Value<decimal>("receivable") != decimal.Zero)
                {
                    return new { message = "应收余额不为零，不能删除！" };
                }
                var detail = db.FirstOrDefault("select * from bill where (content->>'customerid')::int="+id);
                if (detail!=null)
                {
                    return new { message = "该客户有关联的业务单据，不能删除！" };
                }
        ...
        public Object Stop(string parameters)
        {
            ...
            using (DBHelper db = new DBHelper())
            {
                ...
                if (model.content.Value<decimal>("receivable") != decimal.Zero)
                {
                    return new { message = "应收余额不为零，不能停用！" };
                }
