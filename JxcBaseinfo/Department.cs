            ...
            try
            {
                ...
                using (DBHelper db = new DBHelper())
                {
                    ...
                    for (int i = 1; i < rowcount; i++)
                    {
                        ...
                        department.Add("code", code);
                        department.Add("name", name);
                        department.Add("pycode", PyConverter.IndexCode(name));

                        TableModel model = new TableModel()
                        {
                            content = JsonConvert.DeserializeObject<JObject>(JsonConvert.SerializeObject(department))
                        };
