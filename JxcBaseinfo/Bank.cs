...
{
    public class Bank : MarshalByRefObject
    {
            ...
            try
            {
                ...
                using (DBHelper db = new DBHelper())
                {
                    ...
                    for (int i = 1; i < rowcount; i++)
                    {
                        ...
                        bank.Add("code", code);
                        bank.Add("name", name);
                        bank.Add("pycode", PyConverter.IndexCode(name));
                        if (row.GetCell(2).GetCellValue().ToString() != "")
                        {
                            bank.Add("inittotal", Convert.ToDecimal(row.GetCell(2).GetCellValue()));
                        }

                        TableModel model = new TableModel()
                        {
                            content = JsonConvert.DeserializeObject<JObject>(JsonConvert.SerializeObject(bank))
                        };
        ...
        public Object Delete(string parameters)
        {
            ...
            using (DBHelper db = new DBHelper())
            {
                var bank = db.First("bank",id);
                if (bank.content.Value<decimal>("total") != decimal.Zero)
                {
                    return new { message = "银行账户余额不为零，不能删除！" };
                }
                int cnt = db.Count("select count(0) as cnt from bankdetail where (content->>'bankid')::int="+id
                    +" or (content->>'bankid2')::int="+id);
                if (cnt>0)
                {
                    return new { message = "银行账户有往来明细，不能删除！" };
                }
