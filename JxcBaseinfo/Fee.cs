...
{
    public class Fee : MarshalByRefObject
    {
        private string tablename = "fee";
            ...
            try
            {
                ...
                using (DBHelper db = new DBHelper())
                {
                    ...
                    for (int i = 1; i < rowcount; i++)
                    {
                        ...
                        Dictionary<string, string> fee = new Dictionary<string, string>();
                        fee.Add("code", code);
                        fee.Add("name", name);
                        fee.Add("pycode", PyConverter.IndexCode(name));

                        TableModel model = new TableModel()
                        {
                            content = JsonConvert.DeserializeObject<JObject>(JsonConvert.SerializeObject(fee))
                        };
        ...
        public Object Delete(string parameters)
        {
            ...
            using (DBHelper db = new DBHelper())
            {
                var detail = db.FirstOrDefault("select * from bill from bill "
                    + "where exists(select 1 from jsonb_array_elements(content->'details')  t(x) where (x->>'feeid')::int=" + id + ") ");
                if (detail != null)
                {
