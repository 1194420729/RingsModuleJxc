...
namespace JxcAccounting
{
    public class Common : MarshalByRefObject
    {
        ...
        public Object FeeChoice(string parameters)
        {
            ParameterHelper ph = new ParameterHelper(parameters);
            string filter = ph.GetParameterValue<string>("filter");

            DBHelper db = new DBHelper();
            var list = db.Where("select * from fee where coalesce(content->>'stop','')='' and (content->>'name' ilike '%" + filter + "%' or content->>'code' ilike '%" + filter + "%' or content->>'pycode' ilike '%" + filter + "%')");

            return new { data = list };
        }

        public Object FeeList(string parameters)
        {
            QueryParameter para = ParameterHelper.GetQueryParameters(parameters);
            ...
            int recordcount = 0;
            DBHelper db = new DBHelper();
            List<TableModel> list = db.Query("fee", sb.ToString(), sborder.ToString(), out recordcount);

            return new { resulttotal = recordcount, data = list };
        }

        public Object FeeCategorys(string parameters)
        {
            return CategoryHelper.GetCategoryTreeData("fee");
        }
        
        public Object EarningChoice(string parameters)
        {
            ParameterHelper ph = new ParameterHelper(parameters);
            string filter = ph.GetParameterValue<string>("filter");

            DBHelper db = new DBHelper();
            var list = db.Where("select * from earning where coalesce(content->>'stop','')='' and (content->>'name' ilike '%" + filter + "%' or content->>'code' ilike '%" + filter + "%' or content->>'pycode' ilike '%" + filter + "%')");

            return new { data = list };
        }

        public Object EarningList(string parameters)
        {
            QueryParameter para = ParameterHelper.GetQueryParameters(parameters);
            ...
            int recordcount = 0;
            DBHelper db = new DBHelper();
            List<TableModel> list = db.Query("earning", sb.ToString(), sborder.ToString(), out recordcount);

            return new { resulttotal = recordcount, data = list };
        }

        public Object EarningCategorys(string parameters)
        {
            return CategoryHelper.GetCategoryTreeData("earning");
        }
