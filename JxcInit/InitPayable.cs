...
namespace JxcInit
{
    public class InitPayable : MarshalByRefObject
    {
        ...
        {
            var vendor = JsonConvert.DeserializeObject<TableModel>(parameters);
            ...
            using (DBHelper db = new DBHelper())
            {
                ...
                var p = db.First("select * from vendor where id=" + vendor.id);
                p.content["initpayable"] = vendor.content["initpayable"];
                db.Edit(tablename, p);
                db.SaveChanges();
            }
        ...
        public Object PrepareExcel(string parameters)
        {
            ...
            var vendors = db.Where("select * from vendor where coalesce(content->>'stop','')='' order by content->>'code'");

            DataTable dt = new DataTable();
            dt.Columns.Add("供应商编号", typeof(string));
            dt.Columns.Add("供应商名称", typeof(string));
            dt.Columns.Add("期初应付", typeof(decimal));


            foreach (var vendor in vendors)
            {
                DataRow row = dt.NewRow();
                row["供应商编号"] = vendor.content.Value<string>("code");
                row["供应商名称"] = vendor.content.Value<string>("code");
                if (vendor.content["initpayable"] != null)
                {
                    row["期初应付"] = vendor.content.Value<decimal>("initpayable");
                }
