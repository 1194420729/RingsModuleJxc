...
{
    public class StockInBill : MarshalByRefObject
    {
        private string billname = "stockinbill";
        public Object Init(string parameters)
        {
            ...
            var inouttypes = db.Where("select * from inouttype where content->>'direction'='入库' and coalesce(content->>'stop','')=''");
        ...
        public Object RegisterPrintModel(string parameters)
        {
            ...
            pd.HeaderValue = new Dictionary<string, string>()
            {
                ...
                {"入库类型",inouttype.content.Value<string>("name")},
