...
{
    public class StockMoveBill : MarshalByRefObject
    {
        private string billname = "stockmovebill";
        ...
        public Object LoadBillQuery(string parameters)
        {
            ...
            foreach (string field in myfilter.Keys)
            {
                ...
                else if (field == "stockname2")
                {
                    DataTable dt = db.QueryTable("select id from stock where content->>'name' ilike '%" + f + "%'");
                    string ids = dt.GetIds();
                    sb.AppendFormat(" and (content->>'stockid2')::int in ({0})", ids);
                }
            ...
            foreach (var item in list)
            {
                ...
                item.content.Add("stockname2", db.First("stock", item.content.Value<int>("stockid2")).content.Value<string>("name"));
            }
        ...
        public Object LoadBill(string parameters)
        {
            ...
            var stock2 = db.First("stock", bill.content.Value<int>("stockid2"));
            ...
            return new
            {
                ...
                stockname2 = stock2.content.Value<string>("name"),
                noeditbillcode = noeditbillcode,
                noeditbilldate = noeditbilldate, 
                showcost = showcost
            };
        ...
        public Object RegisterPrintModel(string parameters)
        {
            ...
            pd.HeaderValue = new Dictionary<string, string>()
            {
                ...
                {"出库仓库",stock.content.Value<string>("name")},
                {"入库仓库",stock2.content.Value<string>("name")},
